...
import webapp.entities.AccountRecord.Companion.FIRST_NAME_FIELD
import webapp.entities.AccountRecord.Companion.LAST_NAME_FIELD
...
fun ProblemsModel.serverErrorResponse(error: String)
    ...
    .body(forStatus(INTERNAL_SERVER_ERROR)
        .apply {
...
): Set<Map<String, String?>> = exchange
    .validator
    .run {
        return setOf(
            PASSWORD_FIELD, EMAIL_FIELD, LOGIN_FIELD, FIRST_NAME_FIELD, LAST_NAME_FIELD
        ).map { field ->
            field to validateProperty(this@validate, field)
        }.flatMap { violatedField ->
            violatedField.second.map {
                mapOf(
                    "objectName" to objectName, "field" to violatedField.first, "message" to it.message
                )
            }
        }.toSet()
    }
